...
export function MenuClient({ initialItems, backHref }: { initialItems: MenuItem[], backHref?: string }) {
    const [activeCategory, setActiveCategory] = useState<string>("all")
    const [search, setSearch] = useState("")

    // Dynamically get unique categories from the database items
    const dbCategories = Array.from(new Set(initialItems.map((item) => item.category)))
    const dynamicCategories = [
        { label: "All Items", value: "all" },
        ...dbCategories.map((cat) => ({
            label: cat, // e.g., "Espresso", "Tea", "Pastries"
            value: cat,
        }))
    ]
    ...
    const filteredItems = initialItems.filter((item) => {
        ...
        const matchesSearch = item.name.toLowerCase().includes(search.toLowerCase()) || desc.toLowerCase().includes(search.toLowerCase())
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="relative z-10 flex items-center gap-2">
                                        {categoryIcons[cat.value] || <Coffee className="w-4 h-4" />}
                                        {cat.label}
                                    </span>
                                </button>
                            )
                        ...
                        {filteredItems.map((item, index) => (
                            ...
                            >
                                ...
                                <div className="relative w-full aspect-[4/3] overflow-hidden bg-muted/30">
                                    <Image
                                        src={item.imageUrl || "https://images.unsplash.com/photo-1559525839-b184a4d698c7?auto=format&fit=crop&q=80"}
                                        alt={item.name}
                                        fill
                                    ...
                                    {/* Badge (assuming category acts as a badge for now if you want, or you can add a badge field to schema later) */}
