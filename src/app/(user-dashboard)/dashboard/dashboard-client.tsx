...
export function DashboardClient({ user, points, recentOrders }: DashboardClientProps) {
    const [isQrOpen, setIsQrOpen] = useState(false)
    
    // Extract first name or fallback
    const firstName = user.name ? user.name.split(" ")[0] : "Coffee Lover"
    
    // Calculate tier and progress (0-200 Silver, 200-500 Gold, 500+ Platinum)
                            ...
                            <div className="relative z-10">
                                <div className="flex justify-between items-start mb-12">
                                    <div>
                                        ...
                                        <h2 className="text-5xl font-display font-bold text-primary">{points}<span className="text-2xl text-white ml-2">pts</span></h2>
                                    </div>
                                ...
                                <div>
                                    <div className="flex justify-between text-sm mb-2">
                                        <span className="text-white/80 font-medium">{tier}</span>
                                        {points < 500 ? (
                                            <span className="text-white/60">{nextTierPoints - points} pts to {nextTierName}</span>
                                        ) : (
                                            <span className="text-white/60">Max Tier Reached</span>
                                        )}
                                    </div>
                                    <div className="w-full bg-white/10 rounded-full h-2 mb-6">
                                        <div className="bg-primary h-2 rounded-full" style={{ width: `${Math.min(100, Math.max(0, progress))}%` }} />
                                    </div>
                                    ...
                                    <div key={order.id} className="flex items-start gap-4 p-4 rounded-2xl hover:bg-muted/50 transition-colors border border-transparent hover:border-border">
                                        ...
                                        <div className="flex-1">
                                            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-1">
                                                ...
                                                <span className={`font-bold mt-1 sm:mt-0 ${order.type === 'order' ? 'text-primary' : 'text-red-500'}`}>
                                                    {order.type === 'order' ? '+' : '-'}{order.points} pts
                                                </span>
                                            </div>
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="bg-muted py-3 px-4 rounded-xl inline-flex items-center gap-3 w-full justify-center border border-border border-dashed">
                                    ...
                                    <span className="font-mono font-bold text-foreground">DG-{user.id.toString().padStart(4, '0')}-{new Date().getFullYear()}</span>
                                </div>
